...
# from PR https://github.com/huggingface/lerobot/pull/924
# https://github.com/huggingface/lerobot/pull/924/files
...
def merge_datasets(
    ...
    info_path = os.path.join(source_folders[0], "meta", "info.json")
    print(f"DEBUG INFO === Loading info from {info_path}")
    with open(info_path) as f:
        info = json.load(f)
...
if __name__ == "__main__":
    ...
    SINGLE_DATASETS_PATH = "/Users/timthiele/Documents/MA/Data/dual_cam/local_root/lerobot"
    sources = [os.path.join(SINGLE_DATASETS_PATH, source) for source in os.listdir(SINGLE_DATASETS_PATH)]
    for source in sources:
        print(f"Found source folder: {source}")
    
    OUTPUT_DATASET_PATH = "/Users/timthiele/Documents/MA/Data/dual_cam/local_root/merged_lerobot"
    
    # max_dim=6, default_fps=30 for our case
    merge_datasets(sources, OUTPUT_DATASET_PATH, max_dim=6, default_fps=30)